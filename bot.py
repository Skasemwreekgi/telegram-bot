...
# Configuration
TOKEN = '7572796786:AAHowjwbLsFRbu0xxZ0ux8fQasKs40DXC3I'
WEBHOOK_URL = 'https://telegram-bot-iypa.onrender.com/webhook'
HOST = '0.0.0.0'
PORT = 10000  # Render uses a dynamic port, set via environment variable
...
def check_telegram_api():
    try:
        ...
        logging.error(f"Telegram API connection error: {str(e)}")
        return False
...
def remove_metadata(input_path, output_path):
    try:
        ...
        logging.error(f"Metadata removal error: {str(e)}")
        return False
...
def safe_remove(filename, retries=5, delay=0.5):
    ...
    for attempt in range(retries):
        try:
            ...
            logging.warning(f"Cannot delete {filename}: {str(e)}. Retrying...")
            time.sleep(delay)
...
def compute_image_hashes(image_path):
    try:
        ...
        logging.error(f"Error computing image hashes: {str(e)}")
        return None

# Process image with hashing
def process_image(image_path, max_size_mb=10):
    ...
    try:
        ...
        # Apply global rotation to ensure hash differences
        height, width = image.shape[:2]
        center = (width // 2, height // 2)
        angle = random.uniform(-5, 5)  # Random rotation between -5 and 5 degrees
        M = cv2.getRotationMatrix2D(center, angle, 1.0)
        image = cv2.warpAffine(image, M, (width, height), borderMode=cv2.BORDER_REPLICATE)
        logging.info(f"Applied global rotation: {angle} degrees")
        ...
        if len(faces) > 0:
            blur_kernel = 9  # Increased kernel size for stronger blur
            blur_sigma = 3.0  # Increased sigma for stronger blur
            for (x, y, w, h) in faces:
                face_roi = image[y:y+h, x:x+w]
                blurred_face = cv2.GaussianBlur(face_roi, (blur_kernel, blur_kernel), blur_sigma)
                image[y:y+h, x:x+w] = blurred_face
                rows, cols = face_roi.shape[:2]
                M = np.float32([[1, 0.02, 4], [0.02, 1, 4]])  # Stronger shear
                face_roi = cv2.warpAffine(face_roi, M, (cols, rows), borderMode=cv2.BORDER_REPLICATE)
                image[y:y+h, x:x+w] = face_roi

        alpha = 1.1  # Increased contrast
        beta = 10  # Increased brightness
        noise_percentage = 0.01  # Increased noise
        noise_strength = 5  # Increased noise strength
        shuffle_percentage = 0.005  # Increased pixel shuffling
        ...
        noise = np.random.normal(0, 1.0, image.shape).astype(np.float32)  # Stronger Gaussian noise
